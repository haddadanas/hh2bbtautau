...
class HHModel(MLModel):
    ...
    def setup(self):
        ...
        for i in ["1", "2p45", "0", "5"]:
            if f"{self.cls_name}.score_kl{i}" not in self.config_inst.variables:
                self.config_inst.add_variable(
                    name=f"{self.cls_name}.score_kl{i}",
                    null_value=-1,
                    binning=(10, 0, 1),
                    x_title=r"HH Model output ($\kappa_{\lambda}$=" + i + ")",
                )
...
# usable derivations
hh_model = HHModel.derive("hh_model", cls_dict={
    "folds": 3,
    "model_name": "hh_model",
    ...
    "epochs": 50,
    "input_features": {
        f"res_pdnn_s0_m500_{node}" for node in ["hh", "dy", "tt"]
    } | {
        f"hh.{var}" for var in ["pt", "eta", "mass"]
    },
    "target_features": {
        f"hh_model.score_kl{i}" for i in ["1", "2p45", "0", "5"]
    },
})
