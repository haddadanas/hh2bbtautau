...
def selector_factory(
    selector_name: str,
    additional_uses: set | None = None,
    additional_produces: set | None = None,
    lepton_selection_kwargs: dict | None = None,
    ...
    produces = {
        mc_weight, pdf_weights, murmuf_weights, pu_weight, btag_weights, process_ids,
        cutflow_features, increment_stats,
    }
    if not empty_selection:
        produces |= {trigger_selection, lepton_selection, jet_selection}
    if additional_produces:
        produces |= additional_produces

    if not lepton_selection_kwargs:
        lepton_selection_kwargs = dict()
    ...
    def func(
        ...
        **kwargs,
        ...
        # lepton selection
        events, lepton_results = self[lepton_selection](events, trigger_results, **lepton_selection_kwargs, **kwargs)
        results += lepton_results
        ...
        weight_map = {
            "num_events": Ellipsis,
            "num_events_selected": event_sel,
            "num_events_selected_nobjet": event_sel if empty_selection else event_sel_nob,
        }
        group_map: dict[str, dict[Any]] = {}
        group_combinations = []
        if self.dataset_inst.is_mc:
            weight_map["sum_mc_weight"] = events.mc_weight
            weight_map["sum_mc_weight_selected"] = (events.mc_weight, event_sel)
...
empty_selector = selector_factory(
    selector_name="empty",
