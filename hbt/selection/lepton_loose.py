...
@selector(
    uses={
        ...
        "Electron.pfRelIso03_all", "Electron.mvaIso",
...
@selector(
    uses={
        # nano columns
        "Muon.pt", "Muon.eta", "Muon.phi", "Muon.mediumId", "Muon.tightId", "Muon.looseId",
...
def muon_selection(
    ...
    **kwargs,
    ...
    default_mask = None
    default_indices = ak.without_parameters(events.Muon.charge[:, :0])
    if is_single or is_cross:
        if is_2016:
...
def tau_selection(
    ...
    **kwargs,
    ...
    base_mask = (
        ...
        (events.Tau[get_tau_tagger("e")] >= (wp_config.tau_vs_e.vvvloose if is_any_cross_tau
                                            else wp_config.tau_vs_e.vvvloose)) &
        (events.Tau[get_tau_tagger("mu")] >= (wp_config.tau_vs_mu.vloose if is_any_cross_tau
                                            else wp_config.tau_vs_mu.vloose)) &
    ...
    iso_mask = events.Tau[base_indices][get_tau_tagger("jet")] >= wp_config.tau_vs_jet.vvvloose
