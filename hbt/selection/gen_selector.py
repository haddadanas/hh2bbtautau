...
from columnflow.production.util import attach_coffea_behavior, set_ak_column
...
@selector(
    uses={
        json_filter, met_filters, trigger_selection, mc_weight, lepton_selection, jet_selection,
    ...
    produces={
        trigger_selection, mc_weight, pu_weight, btag_weights, lepton_selection, jet_selection,
        process_ids, cutflow_features, increment_stats, gen_higgs_decay_products,
        IF_DATASET_HAS_LHE_WEIGHTS(pdf_weights, murmuf_weights), "selection_mask",
    },
    exposed=True,
)
def gen_default(
    ...
    **kwargs,
    ...
    if self.dataset_inst.is_mc:
        ...
        events = self[btag_weights](
            events,
            ak.fill_none(jet_results.x.jet_mask, False, axis=-1),
    ...
    results += SelectionResult(
        objects=particle_results.objects,
        aux=particle_results.aux,
    )

    # combine lepton and jet selections and add selection mask to events
    events = set_ak_column(events, "selection_mask", reduce(and_, particle_results.steps.values()))
...
@gen_default.init
def gen_default_init(self: Selector) -> None:
