...
def add_hist_hooks(config: od.Config) -> None:
    ...
    def control_qcd_estimation(task, hists):
        if not hists:
            return hists
        ...
        for group_name in complete_groups:
            ...
            os_noniso_mc = hist_to_num(get_hist(mc_hist, "os_noniso"), "os_noniso_mc")
            ss_noniso_mc = hist_to_num(get_hist(mc_hist, "ss_noniso"), "ss_noniso_mc")
            os_iso_mc = hist_to_num(get_hist(mc_hist, "os_iso"), "os_iso_mc")
            os_noniso_data = hist_to_num(get_hist(data_hist, "os_noniso"), "os_noniso_data")
            ss_noniso_data = hist_to_num(get_hist(data_hist, "ss_noniso"), "ss_noniso_data")
            ...
            # shapes: (SHIFT, VAR)
            os_noniso_qcd = os_noniso_data - os_noniso_mc
            os_iso_qcd = os_iso_data - os_iso_mc
            ss_noniso_qcd = ss_noniso_data - ss_noniso_mc
            ...
            if int_os_noniso_neg.any():
                ...
                shifts = list(map(config.get_shift, shift_ids))
                logger.warning(
                    f"negative QCD integral in os_noniso region for group {group_name} and shifts: "
                    f"{', '.join(map(str, shifts))}",
                )

            # ABCD method
            # shape: (SHIFT, VAR)
            ss_iso_qcd = os_iso_qcd * ((int_ss_noniso / int_os_noniso)[:, None])

            # combine uncertainties and store values in bare arrays
            ss_iso_qcd_values = ss_iso_qcd()
            ss_iso_qcd_variances = ss_iso_qcd(sn.UP, sn.ALL, unc=True)**2

            # define uncertainties
            unc_data = ss_iso_qcd(sn.UP, ["os_noniso_data", "os_iso_data", "ss_noniso_data"], unc=True)
            unc_mc = ss_iso_qcd(sn.UP, ["os_noniso_mc", "os_iso_mc", "ss_noniso_mc"], unc=True)
            unc_data_rel = abs(unc_data / ss_iso_qcd_values)
            unc_mc_rel = abs(unc_mc / ss_iso_qcd_values)
            ...
            ss_iso_qcd_variances[keep_variance_mask] = unc_mc[keep_variance_mask]**2
            ss_iso_qcd_variances[~keep_variance_mask] = 0

            # retro-actively set values to zero for shifts that had negative integrals
            neg_int_mask = int_ss_noniso_neg | int_os_noniso_neg
            ss_iso_qcd_values[neg_int_mask] = 1e-5
            ss_iso_qcd_variances[neg_int_mask] = 0

            # residual zero filling
            zero_mask = ss_iso_qcd_values <= 0
            ss_iso_qcd_values[zero_mask] = 1e-5
            ss_iso_qcd_variances[zero_mask] = 0

            # insert values into the qcd histogram
            cat_axis = qcd_hist.axes["category"]
            for cat_index in range(cat_axis.size):
                if cat_axis.value(cat_index) == group.ss_iso.id:
                    qcd_hist.view().value[cat_index, ...] = ss_iso_qcd_values
                    qcd_hist.view().variance[cat_index, ...] = ss_iso_qcd_variances
                    break
            else:
                raise RuntimeError(
                    f"could not find index of bin on 'category' axis of qcd histogram {qcd_hist} "
                    f"for category {group.ss_iso}",
                )
    ...
    def avoid_plotting(task, hists):
        ...
        for process, h in hists.items():
            h_new = hist.Hist(
                ...
                hist.axes.IntCategory([101], name="category"),
