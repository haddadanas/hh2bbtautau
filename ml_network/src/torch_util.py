...
class BatchedMultiNodeWeightedSampler(MultiNodeWeightedSampler):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if _real_total_size != self.batch_size:
            print(
                "Warning: requested batch size is not equal to the sum of the computed batch composition sizes. "
        ...
        def _weight_check(weight):
            if not isinstance(weight, float) or weight <= 0:
                raise ValueError(
                    ...
                    non-negative, and non-zero. Weights are used to sample from source nodes. Zero weight means the
                    source node will never be sampled from, and can cause unexpected behavior depending on the stop
                    criteris. Weights are used as inputs to torch.multinomial, please refer to
                    https://pytorch.org/docs/stable/generated/torch.multinomial.html
                    on how to use weights for sampling."""
                )
    ...
    def next(self) -> dict[str, list[object]]:
        self._started = True
        ...
        for source_name in self.weights:
            ...
            for _ in range(batch_size):
                ...
                try:
                    ...
                    if (
                        (not self.drop_last and len(sub_batch) > 0) and
